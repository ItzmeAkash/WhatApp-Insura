...
from utils.helpers import extract_image_driving_license, extract_image_info1, extract_image_mulkiya, extract_pdf_driving_license, extract_pdf_info1, extract_pdf_mulkiya, store_interaction

async def process_uploaded_document(from_id: str, document_data: bytes, mime_type: str, filename: str, user_states: dict, flow_type: str = None) -> dict:
    mime_to_ext = {
        "application/pdf": ".pdf",
        "image/jpeg": ".jpg",
        "image/png": ".png"
...
async def display_extracted_info(from_id: str, extracted_info: dict, user_states: dict, flow_type: str = None):
...
async def merge_id_information(from_id: str, back_extracted_info: dict, user_states: dict, flow_type: str = None):
...
async def complete_document_editing(from_id: str, user_states: dict, flow_type: str = None):
...
async def proceed_with_verified_document(from_id: str, user_states: dict):
    ...
    if flow_type == "Medical Insurance":
        ...
        user_states[from_id]["responses"]["motor_member_name"] = verified_info.get("name", "")
        user_states[from_id]["responses"]["motor_member_dob"] = verified_info.get("date_of_birth", "")
        user_states[from_id]["responses"]["motor_member_gender"] = verified_info.get("gender", "")
    ...
    if flow_type == "Medical Insurance":
        ...
        store_interaction(from_id, "Bot asked for vehicle info", license_question, user_states)
...
async def process_uploaded_license_document(from_id: str, document_data: bytes, mime_type: str, filename: str, user_states: dict, flow_type: str = None) -> dict:
    mime_to_ext = {
        "application/pdf": ".pdf",
        "image/jpeg": ".jpg",
        "image/png": ".png"
...
async def display_license_extracted_info(from_id: str, extracted_info: dict, user_states: dict, flow_type: str = None):
    # Define the fields to display
    fields_to_display = [
        "name",
        "license_no",
    ...
    user_states[from_id]["motor_license_verified_info"] = extracted_info.copy()  # Create a copy that can be edited
    ...
    # Send the message with all extracted information
    send_whatsapp_message(from_id, message)
    store_interaction(from_id, "License document information displayed", message, user_states)

    # Ask if the information is correct
    user_states[from_id]["stage"] = "lience_document_info_confirmation"
    confirmation_question = "Is all the information correct?"
    send_yes_no_options(from_id, confirmation_question, user_states)
    store_interaction(from_id, "Bot asked for license information confirmation", confirmation_question, user_states)
    
async def merge_id_license_information(from_id: str, back_extracted_info: dict, user_states: dict, flow_type: str = None):
    # Retrieve the front side information
...
async def handle_lience_document_edit(from_id: str, user_states: dict, field_to_edit=None, new_value=None):
    # Define the fields that can be edited
    editable_fields =   [
                   "name",
            "license_no",
    ...
    # If user has selected a field to edit
        ...
        if field_key in editable_fields:
            ...
            current_value = user_states[from_id]["motor_license_verified_info"].get(field_key, "")
            edit_prompt = f"The current value for {field_to_edit} is: {current_value}\n\nPlease enter the new value:"
...
async def complete_licence_document_editing(from_id: str, user_states: dict, flow_type: str = None):
    # Update member information with the verified info
...
async def proceed_with_license_verified_document(from_id: str, user_states: dict, flow_type: str = None):
    motor_license_verified_info = user_states[from_id]["motor_license_verified_info"]
    
    # Update member information with the verified info

    user_states[from_id]["responses"]["motor_licence_name"] = motor_license_verified_info.get("name", "")
    user_states[from_id]["responses"]["motor_licence_dob"] = motor_license_verified_info.get("date_of_birth", "")
    user_states[from_id]["responses"]["motor_licence_license_no"] = motor_license_verified_info.get("license_no", "")
...
async def proceed__license_without_edits(from_id: str, user_states: dict, flow_type: str = None):
    motor_license_verified_info = user_states[from_id]["motor_license_verified_info"]
    ...
    store_interaction(from_id, "License information confirmed", confirmation_message, user_states)
    ...
    store_interaction(from_id, "Bot asked for vehicle mulkiya", mulkiya_question, user_states)
...
async def process_uploaded_mulkiya_document(from_id: str, document_data: bytes, mime_type: str, filename: str, user_states: dict, flow_type: str = None) -> dict:
    mime_to_ext = {
        "application/pdf": ".pdf",
        "image/jpeg": ".jpg",
        "image/png": ".png"
...
async def display_mulkiya_extracted_info(from_id: str, extracted_info: dict, user_states: dict, flow_type: str = None):
    # Define the fields to display
    fields_to_display = [
         "owner",
         "traffic_plate_no",
    ...
    user_states[from_id]["motor_mulkiya_verified_info"] = extracted_info.copy()  # Create a copy that can be edited
    ...
    # Send the message with all extracted information
    send_whatsapp_message(from_id, message)
    store_interaction(from_id, "Mulkiya document information displayed", message, user_states)

    # Ask if the information is correct
    user_states[from_id]["stage"] = "mulkiya_document_info_confirmation"
    confirmation_question = "Is all the information correct?"
    send_yes_no_options(from_id, confirmation_question, user_states)
    store_interaction(from_id, "Bot asked for mulkiya information confirmation", confirmation_question, user_states)
    
async def merge_id_mulkiya_information(from_id: str, back_extracted_info: dict, user_states: dict, flow_type: str = None):
    # Retrieve the front side information
...
async def handle_mulkiya_document_edit(from_id: str, user_states: dict, field_to_edit=None, new_value=None):
    # Define the fields that can be edited
    editable_fields =   [
         "owner",
         "traffic_plate_no",
    ...
    # If user has selected a field to edit
        ...
        if field_key in editable_fields:
            ...
            current_value = user_states[from_id]["motor_mulkiya_verified_info"].get(field_key, "")
            edit_prompt = f"The current value for {field_to_edit} is: {current_value}\n\nPlease enter the new value:"
...
async def complete_mulkiya_document_editing(from_id: str, user_states: dict, flow_type: str = None):
    # Update member information with the verified info
...
async def proceed_with_mulkiya_verified_document(from_id: str, user_states: dict, flow_type: str = None):
    motor_mulkiya_verified_info = user_states[from_id]["motor_mulkiya_verified_info"]
    
    # Update member information with the verified info

    user_states[from_id]["responses"]["motor_mulkiya_owner_name"] = motor_mulkiya_verified_info.get("owner", "")
    user_states[from_id]["responses"]["motor_mulkiya_traffic_plate_no"] = motor_mulkiya_verified_info.get("traffic_plate_no", "")
    user_states[from_id]["responses"]["motor_mulkiya_tc_no"] = motor_mulkiya_verified_info.get("tc_no", "")
    user_states[from_id]["responses"]["motor_mulkiya_nationality"] = motor_mulkiya_verified_info.get("nationality", "")
...
async def proceed__mulkiya_without_edits(from_id: str, user_states: dict, flow_type: str = None):
    try:
        ...
        # Send confirmation message
        confirmation_message = "Thank you for confirming your Vehicle Mulkiya information."
        send_success = send_whatsapp_message(from_id, confirmation_message)
        store_interaction(from_id, "Mulkiya information confirmed", confirmation_message, user_states)
        
        if not send_success:
            print(f"Failed to send confirmation message to {from_id}")
            send_whatsapp_message(from_id, "There was an issue processing your request. Please try again later.")
            return
        ...
        options_success = send_interactive_options(from_id, wish_to_buy_question, valid_wish_to_buy, user_states)
        
        if options_success:
            store_interaction(from_id, "Bot asked for insurance type to buy", wish_to_buy_question, user_states)
            print(f"Interactive options sent successfully to {from_id}: {valid_wish_to_buy}")
        else:
            ...
            store_interaction(from_id, "Fallback prompt for insurance type", fallback_message, user_states)
        ...
        store_interaction(from_id, "Error in mulkiya processing", f"Error: {str(e)}", user_states)